...
proc watres {outname CAT} {
  ...
  for {set i 0} {$i < $numf} {incr i} {
    ...
    set poreIon [atomselect top "water and same residue as ((abs(z) < 70) and (x^2 + y^2 < 500) and within 5 of (name $CAT))"]
    set bulk    [atomselect top "water and same residue as (abs(z) > 70)"]
    set bulkIon [atomselect top "water and same residue as ((abs(z) > 70) and within 5 of (name $CAT))"]
