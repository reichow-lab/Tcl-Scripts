...
## 	Creates the config files
	...
	foreach ending $suffix cfg $configs {
		...
		if {$ending == "_LipMelt"} {
			puts $ofile [format "set temperature\t\t300\n"]
			puts $ofile "if \{0\} \{"
		} else {
			puts $ofile [format "set temperature\t\t310\n"]
			puts $ofile "if \{1\} \{"
		}
		...
		puts $ofile [format "parameters\t\t/home/bassamh/Topology\\ and\\ Parameters/par_all36m_prot.prm"]
		...
		puts $ofile [format "#parameters\t\t/home/bassamh/Topology\\ and\\ Parameters/gsh.prm\n"]

		if {$ending == "_LipMelt"} {
			puts $ofile "if \{1\} \{"
		} else {puts $ofile "if \{0\} \{"
		}
		...
		if {$ending == "_Equil" || $ending == "_Produc"} {
			puts $ofile [format "timestep\t\t2.0"]
		} else {puts $ofile [format "timestep\t\t1.0"]
		}
		...
		if {$ending == "_ProtConst" || $ending == "_BackConst"} {
			puts $ofile [format "useConstantArea\t\tno ;# no for water box, yes for membrane\n"]
		} else {puts $ofile [format "useConstantArea\t\tyes ;# no for water box, yes for membrane\n"]
		}
		...
		puts $ofile "# Steered restraint for selection"
		...
		puts $ofile [format "fixedAtomsFile\t\t$outname$end1"]
		...
		puts $ofile [format "constraints\t\ton"]
		puts $ofile [format "consexp\t\t\t2"]
		if {$ending == "_ProtConst"} {
			puts $ofile [format "consref\t\t\t$outname$end2"]
			puts $ofile [format "conskfile\t\t$outname$end2"]
		} else {puts $ofile [format "consref\t\t\t$outname$end3"]
			puts $ofile [format "conskfile\t\t$outname$end3"]
		}
